...
export default function Mapper({ currentPos, trucks, selectedTruckId }: MapperProps) {
  ...
  return (
    ...
    >
      ...
      {trucks.map((truck) => {
        const truckImage = selectedTruckId === truck.id ? 'bullseye.png' : 'truck.png'
        return (
          <Marker key={truck.id} position={[truck.lat, truck.lon]} icon={MapIcon(truckImage)}>
            <Popup>
              {truck.name}
              <br />
              {truck.description}
